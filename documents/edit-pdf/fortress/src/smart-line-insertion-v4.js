...
const SYNTHETIC_FLAG=Symbol('alltoolforest-smart-line-v4');
...
function buildVisualLines(state,layer){
  ...
  for(const block of state?.analysis?.blocks||[]){
    ...
    for(let lineIndex=0;lineIndex<lines.length;lineIndex++){
      ...
      entries.push({
        kind:'original',key:`block:${block.id}:${lineIndex}`,block,lineIndex,text:line?.text||text,style,
        xPdf:x,yPdf:y,left:Math.min(p.x,pr.x),right:Math.max(p.x,pr.x),baselineY:p.y,
        top:p.y-fontPx*.92,bottom:p.y+fontPx*.30,fontPx,pageIndex:block.pageIndex,
      });
    }
  }
  ...
  for(let txIndex=0;txIndex<transactions.length;txIndex++){
    ...
    for(let lineIndex=0;lineIndex<renderedCount;lineIndex++){
      ...
      entries.push({
        kind:'inserted',key:`tx:${tx.id}:${lineIndex}`,tx,txIndex,lineIndex,
        text:textLines[Math.min(lineIndex,textLines.length-1)]||tx.replacementUnicode||'',style,
        xPdf:x,yPdf:y,left:Math.min(p.x,pr.x),right:Math.max(p.x,pr.x),baselineY:p.y,
        top:p.y-fontPx*.92,bottom:p.y+fontPx*.30,fontPx,pageIndex:tx.pageIndex,
      });
    }
  }
...
function referenceForHit(hit,state,layer,entries){
  if(hit?.classList?.contains('pdf-hit-inserted')){
    ...
    const tx=txs[index];
    if(!tx)return null;
    const candidates=entries.filter(entry=>entry.kind==='inserted'&&entry.tx?.id===tx.id);
...
function referenceFromBlankClick(layer,event,entries){
  ...
  const list=candidates.filter(item=>item.list);
...
function estimateSpacing(reference,entries){
  if(reference.kind==='inserted'){
    ...
    if(Number.isFinite(h)&&h>0){
      const matrixScale=reference.fontPx/Math.max(reference.style.fontSize,1);
      return Math.max(reference.fontPx*.95,h*matrixScale);
    }
  }
...
function blocksBelowInsertion(state,entries,insertedBottom,insertedLeft,insertedRight){
  const ids=new Set();
  for(const entry of entries){
    if(entry.kind!=='original'||!entry.block)continue;
    if(entry.top<=insertedBottom+.5)continue;
    if(overlap(insertedLeft,insertedRight,entry.left,entry.right)<Math.min(8,Math.max(1,entry.right-entry.left)*.2))continue;
    ids.add(entry.block.id);
  }
...
function classifyWhitespace(app,state,input,layer,anchor){
  ...
  const below=entries.filter(entry=>entry.top>insertedBottom+.5&&overlap(insertedLeft,insertedRight,entry.left,entry.right)>=Math.min(8,Math.max(1,entry.right-entry.left)*.2)).sort((a,b)=>a.top-b.top);
  const nearest=below[0]||null;
  if(!nearest)return {passthrough:true};
  const distance=nearest.top-insertedBottom;
  const fits=distance>=safety;
  return {fits,distance,belowBlocks:blocksBelowInsertion(state,entries,insertedBottom,insertedLeft,insertedRight)};
}
...
export function attachSmartLineInsertion({app,getEditor}){
  ...
  function onAddTextCapture(event){
    ...
    const reference=hit?referenceForHit(hit,s,layer,entries):referenceFromBlankClick(layer,event,entries);
    if(!reference)return;
  ...
  function onInsertDoneCapture(event){
    ...
    const classification=classifyWhitespace(app,s,input,layer,anchor);
  ...
  async function convertMultilineEdit(event){
    ...
    try{
      ...
      const entries=buildVisualLines(s,layer);
      const reference=entries.find(entry=>entry.kind==='original'&&entry.block?.id===block.id);
      ...
      insertInput.value=extra;insertInput.dispatchEvent(new Event('input',{bubbles:true}));
      ...
      insertDone.click();
    }catch(error){console.warn('Smart line insertion:',error);}
