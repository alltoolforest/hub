...
function findReferenceFromBlankClick(layer,event,state){
  ...
  for(const block of state.analysis.blocks){
    ...
    const scale=Math.max(Math.abs(matrix[3]||1),.1);
    const fontPx=Math.max(8,style.fontSize*scale);
    const vertical=clickY-p.y;
    if(vertical<fontPx*.15||vertical>fontPx*3.7)continue;
    ...
    const listBonus=isListLike(currentBlockText(block,state))?fontPx*1.35:0;
    const boldPenalty=style.bold&&vertical>fontPx*1.5?fontPx*1.5:0;
    const score=vertical+horizontal*.28+boldPenalty-listBonus;
...
function insertionPointForReference(layer,block,state){
  ...
  const style=inferBlockStyle(block);
  const spacing=estimateLineSpacing(block,state);
  const targetBaselinePdf={x:line.x,y:line.y-spacing};
  const target=applyToPoint(matrix,targetBaselinePdf.x,targetBaselinePdf.y);
  const scaleY=Math.max(Math.abs(matrix[3]||1),.1);
  const editorFontPx=Math.max(16,style.fontSize*scaleY);
  const top=target.y-editorFontPx*.88;
  return {x:target.x,y:top,baselineY:target.y,spacing,style};
}
