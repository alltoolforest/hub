...
export function applyDocumentSafety(blocks,flags){
  ...
  if(flags.hasAcroForm)return blocks.map(b=>b.tier==='DIRECT_EDIT'?({...b,tier:SupportTier.LIMITED_EDIT,confidence:Math.min(b.confidence,.7),reason:'ACROFORM_DOCUMENT_REQUIRES_REVIEW'}):b);
  return blocks;
}
