...
function blockBaseline(block){
  const y=Number(block?.lines?.[0]?.y);
  if(Number.isFinite(y))return y;
  const r=blockRect(block);
  return r?r.bottom+Math.max(1,Number(block?.fontSize)||12)*.24:null;
}
function needsCompaction(tx){
  return tx?.kind==='REPLACE_TEXT'&&!!tx?.block&&
    String(tx?.originalUnicode??tx?.block?.text??'').trim().length>0&&
    String(tx?.replacementUnicode??'').trim().length===0;
}
...
function lineGroups(items,fontSize){
  ...
  for(const rect of rects.sort((a,b)=>b.baseline-a.baseline||a.left-b.left)){
    ...
    if(rect.fontName)line.fontNames.add(rect.fontName);
    line.left=Math.min(line.left,rect.left);line.right=Math.max(line.right,rect.right);
    line.bottom=Math.min(line.bottom,rect.bottom);line.top=Math.max(line.top,rect.top);
    line.fontSize=Math.max(line.fontSize,rect.fontSize);
  }
  return lines.sort((a,b)=>b.baseline-a.baseline);
}
function sameLane(line,source,pageWidth,fontSize){
  if(overlap(line.left,line.right,source.left,source.right)>Math.min(8,Math.max(1,source.width)*.15))return true;
  return source.left<=pageWidth*.30&&Math.abs(line.left-source.left)<=Math.max(24,fontSize*2.2);
}
function sourceLinePitch(lines,tx,source,{width,fontSize,baseline}){
  ...
  const minGap=fontSize*.72,maxGap=fontSize*2.6;
  const candidates=[];
  for(const line of lines){
    ...
    if(gap>=minGap&&gap<=maxGap)candidates.push(gap);
  }
  candidates.sort((a,b)=>a-b);
  return candidates[0]||Math.max(fontSize,fontSize*1.20);
}
function footerGuardTop(lines,{pageHeight,fontSize,bottomMargin}){
  ...
  for(let i=1;i<low.length;i++){
    const line=low[i],gap=line.bottom-clusterTop;
    if(gap>=minGap&&clusterTop<=pageHeight*.16)return clamp(clusterTop+Math.max(5,fontSize*.45),bottomMargin,pageHeight*.18);
...
function inferGeometry(lines,tx,{width,height}){
  ...
  const pitch=sourceLinePitch(lines,tx,source,{width,fontSize,baseline});
  ...
  const bandLeft=clamp(Math.min(source.left,nearest.left)-pad,0,width);
  let bandRight;
  ...
  }else bandRight=clamp(Math.max(source.right,nearest.right)+pad,bandLeft+60,width);
  ...
  // The nearest downstream glyph may safely occupy the deleted line's former
  // glyph box. Cap against glyph geometry, not the whitespace slice boundary.
  const maxSafeShift=Math.max(0,source.top-nearest.top+.5);
  const shiftY=Math.min(pitch,maxSafeShift);
  ...
  const footerTop=footerGuardTop(lines,{pageHeight:height,fontSize,bottomMargin});
...
function inspectAnnotations(doc,page){
  ...
  for(let i=0;i<array.size();i++){
    ...
    const arr=lookup(doc,dict.get(RECT));
    if(!arr||typeof arr.size!=='function'||arr.size()<4)return {ok:false,reason:'LINK_RECTANGLE_MISSING'};
    const rect=[];
    for(let j=0;j<4;j++){const value=numberValue(doc,arr.get(j));if(!Number.isFinite(value))return {ok:false,reason:'LINK_RECTANGLE_INVALID'};rect.push(value);}
    links.push({dict,rect,bounds:rectBounds(rect)});
  }
  return {ok:true,raw,links};
}
function classifyLink(link,g){
  const {left,right,bottom,top}=link.bounds;
  if(top<=g.footerGuardTop+.5)return 'STATIC_FOOTER';
  if(bottom<g.footerGuardTop-.5&&top>g.footerGuardTop+.5)return 'CROSSES_FOOTER';
  if(right<=g.bandLeft+.5||left>=g.bandRight-.5)return 'STATIC';
  if((left<g.bandLeft-.5&&right>g.bandLeft+.5)||(left<g.bandRight-.5&&right>g.bandRight+.5))return 'CROSSES_BAND';
  if(bottom<g.cutY-.5&&top>g.cutY+.5)return ((bottom+top)/2<=g.cutY?'MOVED':'STATIC');
  if(top<=g.cutY+.5)return 'MOVED';
  return 'STATIC';
}
function validateLinks(info,g,pageHeight){
  const links=[];
  for(const link of info.links){
    const zone=classifyLink(link,g);
    ...
    if(zone==='MOVED'&&link.bounds.top+g.shiftY>pageHeight-1)return {ok:false,reason:'LINK_COMPACTION_OUT_OF_PAGE'};
    links.push({...link,zone});
  }
  return {ok:true,links};
}
function preserveLinks(doc,replacement,info,links,shiftY){
  if(!info.raw)return;
...
async function vectorSafety(bytes,pageIndex,g){
  try{
    ...
    const pdf=await task.promise,page=await pdf.getPage(pageIndex+1),ops=await page.getOperatorList();
    const bad=[];
    for(let i=0;i<ops.fnArray.length;i++){
      if(ops.fnArray[i]!==p.OPS.constructPath)continue;
      const raw=ops.argsArray[i]?.[2];if(!raw)continue;
      ...
      const vertical=w<=2.5&&h>=10&&((x0+x1)/2)>=g.bandLeft-1&&((x0+x1)/2)<=g.bandRight+1;
    ...
    return bad.length?{ok:false,reason:'STRUCTURED_VECTOR_REFLOW_UNSAFE',boundaries:bad}:{ok:true};
  }catch(error){return {ok:false,reason:'VECTOR_REFLOW_PREFLIGHT_FAILED',error:String(error)};}
}
async function pageLines(bytes,pageIndex,fontSize){
  const p=await loadPdfjs();
  const task=p.getDocument({data:bytes.slice(),isEvalSupported:false,useWorkerFetch:false,disableFontFace:true});
  const pdf=await task.promise;
  try{const page=await pdf.getPage(pageIndex+1),tc=await page.getTextContent();return lineGroups(tc.items,fontSize);}
  finally{try{await pdf.destroy?.();}catch{}try{await task.destroy?.();}catch{}}
}
async function embed(doc,donorPage,{left=0,bottom=0,right,top}){return top>bottom&&right>left?doc.embedPage(donorPage,{left,bottom,right,top}):null;}
...
async function compactOne(doc,tx,sequenceIndex){
  ...
  const page=doc.getPage(pageIndex),rotation=((page.getRotation().angle||0)%360+360)%360;
  if(rotation!==0)return {applied:false,reason:'ROTATED_PAGE_REFLOW_UNSUPPORTED'};
  const {width,height}=page.getSize(),annotations=inspectAnnotations(doc,page);
  if(!annotations.ok)return {applied:false,reason:annotations.reason};
  const donorBytes=new Uint8Array(await doc.save({useObjectStreams:false,addDefaultPage:false,updateFieldAppearances:false}));
  const lines=await pageLines(donorBytes,pageIndex,Math.max(6,Number(tx.block?.fontSize)||12));
  const g=inferGeometry(lines,tx,{width,height});if(!g.ok)return {applied:false,reason:g.reason};
  ...
  const footer=g.footerGuardTop>.5?await embed(doc,donor,{left:0,bottom:0,right:width,top:g.footerGuardTop}):null;
  const moving=await embed(doc,donor,{left:g.bandLeft,bottom:g.footerGuardTop,right:g.bandRight,top:g.cutY});
  const left=g.bandLeft>1?await embed(doc,donor,{left:0,bottom:g.footerGuardTop,right:g.bandLeft,top:g.cutY}):null;
  const right=g.bandRight<width-1?await embed(doc,donor,{left:g.bandRight,bottom:g.footerGuardTop,right:width,top:g.cutY}):null;
  const replacement=doc.insertPage(pageIndex,[width,height]);
  draw(replacement,top,{x:0,y:g.cutY,width,height:height-g.cutY});
  draw(replacement,footer,{x:0,y:0,width,height:g.footerGuardTop});
  draw(replacement,left,{x:0,y:g.footerGuardTop,width:g.bandLeft,height:g.cutY-g.footerGuardTop});
  draw(replacement,right,{x:g.bandRight,y:g.footerGuardTop,width:width-g.bandRight,height:g.cutY-g.footerGuardTop});
  draw(replacement,moving,{x:g.bandLeft,y:g.footerGuardTop+g.shiftY,width:g.bandWidth,height:g.cutY-g.footerGuardTop});
  preserveLinks(doc,replacement,annotations,links.links,g.shiftY);doc.removePage(pageIndex+1);
...
export async function applyUpwardCompaction(result,transactions,{preview=false}={}){
  const candidates=(transactions||[]).filter(needsCompaction);
  ...
  const pages=new Set(candidates.map(tx=>Number(tx.pageIndex)));
  if((transactions||[]).some(tx=>tx?.kind==='INSERT_TEXT'&&pages.has(Number(tx.pageIndex))))throw Object.assign(new Error('Finish line deletion compaction before adding new text on the same page.'),{code:'COMPACTION_WITH_INSERT_SAME_PAGE_UNSUPPORTED'});

  const doc=await PDFDocument.load(copyBytes(result.bytes),{ignoreEncryption:true,updateMetadata:false});
  const metrics=[...(result?.reflowMetrics||[])],warnings=[...(result?.warnings||[])];
  // PDF Y grows upward. Bottom-most deleted lines must compact first so every
  // later source coordinate still has an unchanged upstream spacing reference.
  ...
  let appliedCount=0,totalShift=0;
  for(const tx of ordered){
    const sequenceIndex=(transactions||[]).findIndex(item=>item?.id===tx.id);
    const applied=await compactOne(doc,tx,sequenceIndex);
    if(!applied.applied){
    ...
    appliedCount++;totalShift+=Number(applied.metric.shiftY)||0;metrics.push(applied.metric);
    warnings.push({code:'LAYOUT_COMPACTED_UPWARD',pageIndex:Number(tx.pageIndex),transactionId:tx.id,shiftY:applied.metric.shiftY,linePitch:applied.metric.linePitch,message:'Content below the deleted line moved upward by one measured visual line advance.'});
  }
